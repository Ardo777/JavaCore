...
public class Main implements Commands {
    ...
    private static void buy_products(Product product, double sum, int count) {
        ...
        if (answer.toUpperCase().equals("YES")) {
            orderStorage.addOrder(new Order(onlineUser, product, OrderStatus.NEW, new Date(), sum, count, PaymentMethod.valueOf(paymentMethod)));
    ...
    private static void register() {
        ...
        if (typeUser.equals(Type.USER.name()) || typeUser.equals(Type.ADMIN.name())) {
            User user = new User(username, email, password, phoneNumber, Type.valueOf(typeUser));
